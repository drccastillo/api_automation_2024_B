...
@when(u'I call to "{endpoint}" endpoint using "GET" method and without body')
def call_get_endpoint(context, endpoint):
    LOGGER.debug(f"STEP: When I call to '{endpoint}' endpoint using 'GET' method and without body")
...
@when(u'I call to "{endpoint}" endpoint using "POST" method and with body')
def call_post_endpoint(context, endpoint):
    LOGGER.debug(f"STEP: When I call to '{endpoint}' endpoint using 'POST' method and with body")
...
@when(u'I call to "{endpoint}" endpoint using "PUT" method and with body')
def call_put_endpoint(context, endpoint):
    LOGGER.debug(f"STEP: When I call to '{endpoint}' endpoint using 'PUT' method and with body")
...
@when(u'I call to "{endpoint}" endpoint using "DELETE" method and without body')
def call_delete_endpoint(context, endpoint):
    LOGGER.debug(f"STEP: When I call to '{endpoint}' endpoint using 'DELETE' method and without body")
...
def receive_response(context, json_file):
    LOGGER.debug(f"STEP: Then I receive the response and validate using '{json_file}' json")
    context.validate.validate_response(
        actual_response=context.response, endpoint=context.endpoint, file_name=json_file
    )
